...
#define waitforevent() SDL_Event _E_; SDL_WaitEvent(&_E_);
#define pollevents() SDL_Event _E_; SDL_PollEvent(&_E_);

//ifev is open-ended, capturing the 'then' and 'else' statements following it, so it's usable just like the regular 'if'
#define ifev( _P_ ) if( _IFEV( _E_, _P_ ))
